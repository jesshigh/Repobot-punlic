...
async def help_cmd(client, message):
    if not get_arg(message):
        ...
        if module in HELP_COMMANDS:
            await message.reply(
                HELP_COMMANDS[module].HELP + '\n<b><a href="tg://user?id=6934282635">© GhostX-Mods</a> </b>',
                quote=True,
...
async def menu_inline(client, inline_query):
    ...
    msg = f"<b>ᴏᴡɴᴇᴅ ʙʏ: ɢʜᴏꜱᴛxᴍᴏᴅꜱ </b>\n<b>ᴘʀᴇꜰɪxᴇꜱ: {' '.join(SH)}</b>\n<b>► ᴛᴏᴛᴀʟ ᴍᴏᴅᴜʟᴇs: {len(HELP_COMMANDS)}</b>"
...
async def menu_callback(client, callback_query):
    ...
    top_text = f"<b>ᴏᴡɴᴇᴅ ʙʏ: ɢʜᴏꜱᴛxᴍᴏᴅꜱ </b>\n<b>ᴘʀᴇꜰɪxᴇꜱ: {' '.join(SH)}</b>\n<b>► ᴛᴏᴛᴀʟ ᴍᴏᴅᴜʟᴇs: {len(HELP_COMMANDS)}</b>"
    if mod_match:
        ...
        await callback_query.edit_message_text(
            text=text + '\n<b><a href="tg://user?id=6934282635">© GhostXMods</a> </b>',
