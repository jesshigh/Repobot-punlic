...
async def prem_user(client, message):
    ...
    if get_id in premium:
        return await Tm.edit(f"**dia sudah bisa membuat userbot**" + emoji("done"))
    ...
    if added:
        ...
        await Tm.edit(
            emoji("done") + f" {get_id} telah di aktifkan selama {get_bulan} bulan, silahkan buat userbot di @{bot.me.username}"
        )
